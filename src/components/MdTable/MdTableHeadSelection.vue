...
export default {
  ...
  methods: {
    onChange(val) {
      if (val) {
        this.MdTable.selectedItems = this.selectedItems.concat(
          this.selectable.filter(item => !this.selectedItems.includes(item))
        );
      } else {
        this.MdTable.selectedItems = this.selectedItems.filter(
          item => !this.selectable.includes(item)
        );
      }
    }
  }
