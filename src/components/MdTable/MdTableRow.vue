<template>
  <tr class="md-table-row" :class="rowClasses" @click="onClick" v-on="$listeners">
    <md-table-cell-selection :value="isMultipleSelected" @input="selected => selected ? addSelection() : removeSelection()" :md-disabled="mdDisabled" :md-selectable="mdSelectable === 'multiple'" :md-row-id="mdIndex" v-if="selectableCount" />
    <slot />
  </tr>
</template>
  ...
  computed: {
    ...
    rowClasses() {
      if (this.MdTable.hasValue) {
        return {
          "md-has-selection":
            !this.mdDisabled && (this.mdAutoSelect || this.hasSingleSelection),
  ...
  methods: {
    ...
    addSelection() {
      if (!this.isMultipleSelected) {
        this.MdTable.selectedItems = this.MdTable.selectedItems.concat([
          this.mdItem
        ]);
      }
    },
    removeSelection() {
      if (this.isMultipleSelected) {
        this.MdTable.selectedItems = this.MdTable.selectedItems.filter(
          target => target !== this.mdItem
        );
      }
    },
    selectRowIfSingle() {
    ...
    removeSelectableItem(target = this.mdItem) {
      if (this.mdSelectable === "multiple") {
        this.MdTable.selectable = this.MdTable.selectable.filter(
          item => item !== target
        );
      }
    }
