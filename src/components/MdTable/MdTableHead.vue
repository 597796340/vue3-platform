...
<script>
/* tslint:disable */
import MdUpwardIcon from "../icons/MdUpwardIcon";
import MdResizeObserver from "./utils/MdResizeObserver";
  ...
  methods: {
    ...
    getChildNodesBySelector(el, selector) {
      return Array.from(el.childNodes).filter(
        ({ classList }) => classList && classList.contains(selector)
      );
    },
    ...
    setWidth() {
      if (this.MdTable.fixedHeader) {
        ...
        const thEls = this.getChildNodesBySelector(
          this.$el.parentNode,
          "md-table-head"
        );
        const tdEls = this.MdTable.contentEl.querySelectorAll(
          "tr:first-child ." + cellSelector
        );
